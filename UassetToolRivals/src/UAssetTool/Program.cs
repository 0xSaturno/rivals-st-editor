...
public class Program
{
    ...
    private static UAssetResponse ProcessRequest(UAssetRequest request)
    {
        try
        {
            return request.Action switch
            {
                ...
                "export_to_json" => ExportToJson(request.FilePath, request.UsmapPath, request.OutputPath),
    ...
    /// </summary>
    private static UAssetResponse ExportToJson(string? filePath, string? usmapPath, string? outputPath = null)
    {
        if (string.IsNullOrEmpty(filePath))
            return new UAssetResponse { Success = false, Message = "File path is required" };
        ...
        // Use custom output path if provided, otherwise save next to the uasset file
        string jsonPath = !string.IsNullOrEmpty(outputPath) ? outputPath : Path.ChangeExtension(filePath, ".json");
